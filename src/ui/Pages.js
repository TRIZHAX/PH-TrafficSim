...
export class Pages {
  ...
  /**
   * Driver analytics panel (brief §15). Reads DrivingMonitor.summary() — a pure
   * read of live session metrics — and renders a metric grid, safety-event
   * tally and recent-event log. When no drive session has produced distance it
   * shows guidance instead. Emoji-free, matching the driving HUD aesthetic.
   */
  _renderDriveAnalytics(host) {
    const mon = this.engine.driving;
    const s = mon ? mon.summary() : null;
    if (!s || s.driveTime < 0.5) {
      host.innerHTML = `<h3>Driver Analytics</h3>
        <p style="font-size:12px;color:var(--text-dim);line-height:1.55">No driving session recorded yet.
        Spawn and drive a vehicle (DRIVE button) to log distance, speed, lane and
        direction discipline, collisions, near-misses and traffic-signal
        compliance. These metrics are also appended to the CSV export as
        <code>driver_*</code> columns.</p>`;
      return;
    }
    const scoreColor = s.score >= 80 ? 'var(--green)' : s.score >= 55 ? 'var(--amber)' : 'var(--red)';
    const mm = String(Math.floor(s.driveTime / 60)).padStart(2, '0');
    const ss = String(Math.floor(s.driveTime % 60)).padStart(2, '0');
    const metric = (k, v, unit = '') => `<div class="an-drive-metric"><span class="k">${k}</span><span class="v">${v}<small>${unit}</small></span></div>`;
    const evt = (e) => {
      const tone = { critical: 'var(--red)', high: 'var(--orange)', medium: 'var(--amber)', low: 'var(--cyan)' }[e.severity] || 'var(--text-dim)';
      const t = `${String(Math.floor(e.t / 60)).padStart(2, '0')}:${String(Math.floor(e.t % 60)).padStart(2, '0')}`;
      return `<div class="an-drive-evt"><span class="dot" style="background:${tone}"></span>
        <span class="code" style="color:${tone}">${e.code || e.type}</span>
        <span class="det">${e.detail || ''}</span><span class="ts">${t}</span></div>`;
    };
    host.innerHTML = `
      <div class="an-drive-head">
        <h3>Driver Analytics</h3>
        <div class="an-drive-score" style="border-color:${scoreColor};color:${scoreColor}">
          <span class="num">${s.score}</span><span class="cap">SCORE</span></div>
      </div>
      <div class="an-drive-grid">
        ${metric('Distance', s.distanceKm, ' km')}
        ${metric('Drive Time', `${mm}:${ss}`, '')}
        ${metric('Avg Speed', s.avgSpeed, ' km/h')}
        ${metric('Max Speed', s.maxSpeed, ' km/h')}
        ${metric('Violations / km', s.violationsPerKm, '')}
        ${metric('Intersections', s.intersectionsEntered, '')}
      </div>
      <div class="an-drive-tally">
        ${this._tally('Collisions', s.collisions, s.collisions ? 'var(--red)' : null, `${s.collisionsVehicle} veh · ${s.collisionsObject} obj`)}
        ${this._tally('Near Misses', s.nearMisses, s.nearMisses ? 'var(--amber)' : null)}
        ${this._tally('Off Road', s.offRoadEvents, s.offRoadEvents ? 'var(--orange)' : null, `${s.offRoadTime}s`)}
        ${this._tally('Wrong Lane', s.wrongLaneEvents, s.wrongLaneEvents ? 'var(--amber)' : null, `${s.wrongLaneTime}s`)}
        ${this._tally('Wrong Way', s.wrongWayEvents, s.wrongWayEvents ? 'var(--red)' : null, `${s.wrongWayTime}s`)}
        ${this._tally('Red-Light', s.redLightViolations, s.redLightViolations ? 'var(--red)' : null)}
        ${this._tally('Lane Departures', s.laneDepartureEvents, s.laneDepartureEvents ? 'var(--cyan)' : null)}
        ${this._tally('Hard Braking', s.hardBrakes, s.hardBrakes ? 'var(--cyan)' : null)}
        ${this._tally('Rapid Accel', s.suddenAccels, s.suddenAccels ? 'var(--cyan)' : null)}
        ${this._tally('Lane Changes', s.laneChanges, null)}
      </div>
      ${s.recentEvents.length ? `<div class="an-drive-log">
        <div class="log-cap">Recent Events</div>
        ${s.recentEvents.map(evt).join('')}
      </div>` : ''}`;
  }

  _tally(label, value, color, sub) {
    return `<div class="an-drive-stat${color ? ' hot' : ''}">
